...
onMounted(() => {
  if (route.path === '/' || route.path === '/fr') localStorage.setItem('lang', 'fr');
  if (route.path === '/en') localStorage.setItem('lang', 'en');
...
<template>
  ...
  <header class="z-10 absolute top-0 left-0 flex justify-between w-full p-4 sm:p-8">
    ...
    <nav class="text-[10px] sm:text-xs">
      <a :class="`heavy-rain-text mr-2 sm:mr-4 ${locale === 'fr' ? 'border-b-2 border-secondary' : ''}`" href="/">FR</a>
      <a :class="`heavy-rain-text ${locale === 'en' ? 'border-b-2 border-secondary' : ''}`" href="/en">EN</a>
    </nav>
  </header>
